...
if (!wordToSearch) {
  console.error('Error: Please provide a word to search for using the "-w" flag.');
  showHelp();
  process.exit(1);
}
...
function searchWordInJson(jsonArray, word) {
  ...
  return jsonArray.filter(item => {
    const titleContainsWord = item.title && item.title.toLowerCase().includes(lowerCaseWord);
    const bodytextContainsWord = item.bodyText && item.bodyText.toLowerCase().includes(lowerCaseWord);
...
const mapped= searchResults.map(item => item.title)
console.log(mapped);